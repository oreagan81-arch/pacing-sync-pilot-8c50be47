  ...
  try {
    ...
    if (!isSynthetic) {
      if (subject === "History" || subject === "Science") {
        blockedReason = `${subject} — assignments are never created (rule)`;
      } else if (day === "Friday" && type !== "Test") {
        blockedReason = "Friday — no homework assignments (rule)";
      } else if (
        subject === "Language Arts" &&
        type && !["CP", "Classroom Practice", "Test"].includes(type)
      ) {
    ...
    if (blockedReason) {
      await sb.from("deploy_log").insert({
        ...
        payload: { day, type, subject, title } as unknown as Record<string, unknown>,
      });
      await sb.from("deploy_notifications").insert({
        level: "warn",
        title: `Assignment blocked — ${subject || ""}`,
